...
const PuzzleCompletedModal = ({
  ...
  const onShare = async (event: MouseEvent, isMobile: boolean) => {
    const target = event.target as HTMLElement
    const realInner = target.querySelector('span') || target
    const origText = realInner.textContent
    ...
    const options = {
      ...
      filter: filter,
      canvasWidth: 1500,
      canvasHeight: isMobile ? 2600 : 1600,
    }
    if (!poster) {
      return
    }
      ...
      >
        ...
        <Flex direction="row" justifyContent="space-between" id="share-button">
          <Button
            ...
            background="transparent"
            color="black"
            onClick={(e) => onShare(e as unknown as MouseEvent, isMobile)}
          >
          ...
          <Button
            ...
            color="black"
            onClick={
              onShareUrl as unknown as MouseEventHandler<HTMLButtonElement>
            }
          >
            <Flex gap="4px" fontWeight="semibold">
